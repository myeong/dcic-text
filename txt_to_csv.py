...
txt_file = r"/Users/myeong/git/DCIC/0330_Python_Session/a-001.txt"
out_file = r"/Users/myeong/git/DCIC/0330_Python_Session/a-001.json"
out_csv = r"/Users/myeong/git/DCIC/0330_Python_Session/a-001.csv"
...
with open(txt_file) as f:
    for line in f:
        ...
        if line.startswith("Security Map of"):
            section = 1
            parsed_data['0.b'] = ' '.join(f.next().strip().split())
        elif line.startswith("a. Increasing"):
            parsed_data['1.a.1'] = ' '.join(f.next().strip().split())
        elif line.startswith("Decreasing"):
            parsed_data['1.a.2'] = ' '.join(f.next().strip().split())
        elif line.startswith("Static"):
            parsed_data['1.a.3'] = ' '.join(f.next().strip().split())
        elif line.startswith("b. Class and Occupation"):
            parsed_data['1.b'] = ' '.join(f.next().strip().split())
        elif line.startswith("c. Foreign Families"):
            parsed_data['1.c.1'] = ' '.join(f.next().strip().split())
        elif line.startswith("Nationalities"):
            parsed_data['1.c.2'] = ' '.join(f.next().strip().split())
        elif line.startswith("d. Negro"):
            datum = f.next().strip() # 0 %
        ...
        # Two-line reading
            ...
            parsed_data['2.o.1'] = ' '.join(f.next().strip().split())
            k = f.next()
            parsed_data['2.o.2'] = ' '.join(k.strip().split())
            
        elif line.startswith("p. Predicted Rent Trend"):    
            parsed_data['2.p.1'] = ' '.join(f.next().strip().split())
            k = f.next()
            parsed_data['2.p.2'] = ' '.join(k.strip().split())
        elif line.startswith("3. NEW CONSTRUCTION"):    
            parsed_data['3.a'] = ' '.join(f.next().strip().split())
        elif line.startswith("Type & Price"):
            parsed_data['3.b'] = ' '.join(f.next().strip().split())
        elif line.startswith("How Selling"):
            parsed_data['3.c'] = ' '.join(f.next().strip().split())
            section = 4
        elif section == 4: 
            if line.startswith("a. HOLC"):
                parsed_data['4.a'] = ' '.join(f.next().strip().split())
            elif line.startswith("b. Institutions"):
                parsed_data['4.b'] = ' '.join(f.next().strip().split())
                section = 5
        elif section == 5: 
            if line.startswith("5. SALE OF HOME PROPERTIES"):
                # How do you extract only the digit?
                parsed_data['5'] = ' '.join(f.next().strip().split())
            elif line.startswith("a. HOLC"):
                parsed_data['5.a'] = ' '.join(f.next().strip().split())
            elif line.startswith("b. Institutions"):
                parsed_data['5.b'] = ' '.join(f.next().strip().split())
            section = 6
        
        elif line.startswith("6. MORTGAGE FUNDS"):
            parsed_data['6'] = ' '.join(f.next().strip().split())
        elif line.startswith("7. TOTAL TAX RATE PER $1000"):
            parsed_data['7.a'] = ' '.join(f.next().strip().split())
            k = f.next()
            parsed_data['7.b'] = ' '.join(k.strip().split())
        elif line.startswith("8. DESCRIPTION AND CHARACTERISTICS OF AREA"):
            parsed_data['8'] = ' '.join(f.next().strip().split()) 
            section = 9
        elif section == 9:
            if line.startswith("9. LOCATION"):
                parsed_data['9.a'] = ' '.join(f.next().strip().split()) 
            elif line.startswith("SECURITY GRADE"):
                parsed_data['9.b'] = ' '.join(f.next().strip().split())
            elif line.startswith("AREA NO."):
                parsed_data['9.c'] = ' '.join(f.next().strip().split())
            elif line.startswith("DATE"):
                parsed_data['9.d'] = ' '.join(f.next().strip().split())
                k=f.next()
...
with open(out_csv, 'wb') as f:  # Just use 'w' mode in 3.x
